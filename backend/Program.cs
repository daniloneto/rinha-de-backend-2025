...
record ProcessorPaymentRequest(Guid CorrelationId, decimal Amount, string RequestedAt);
...
class PaymentWorker : BackgroundService
{
    ...
    private async Task<bool> TrySendAsync(string baseUrl, Payment payment)
    {
        try
        {
            ...
            var req = new ProcessorPaymentRequest(
                payment.CorrelationId,
                payment.Amount,
                payment.RequestedAt.ToString("O"));
            var res = await _http.PostAsJsonAsync(
                $"{baseUrl}/payments",
                req,
                AppJsonContext.Default.ProcessorPaymentRequest,
                cts.Token);
    ...
    private async Task EnsureHealthAsync()
    {
        ...
        try
        {
            ...
            var res = await _http.GetFromJsonAsync<HealthState>(
                $"{_defaultUrl}/payments/service-health",
                AppJsonContext.Default.HealthState,
                cts.Token);
            if (res != null)
                _defaultHealth = res;
        }
